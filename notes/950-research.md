# 950 research design: the questions, what is held fixed, what is varied, and what it costs
...
Written down 2026-09-05 from operator, so that the reason is in the document and not in a conversation. Two things live in `dark`. The **factory** is a software-development pipeline: a model works unattended inside a throwaway virtual machine, pushes a branch, and a hidden judge decides; containment is the VM and the judge, not per-call guards. The **bench** is the factory's measuring instrument: the same runners, the same VMs, isolated Gitea organisations born and archived per experiment, so that a new model's capability can be tested quickly and rigorously, and what is learned there (which tier for which class of task, under which envelope, at what cost) feeds the factory's own choices. The research questions ask whether the factory's approach has the benefit it was built for, against a plain agent session under the same containment.
...
| --- | --- | --- | --- |
| ledger | runner host, `~/.dark`, append-only JSONL | `evidence/950/ledger/<date>.jsonl`, one snapshot per night, copied by the hub before the morning report; a new snapshot must contain the previous one as a prefix | shift id and run id; the report names the snapshot file and its sha256 |
...
| launcher and smoke logs | `/tmp` on the hub, `/tmp/dark-smoke-*`, `/tmp/rq2-*` | `evidence/950/launch/<name>` copied the same morning | file name and sha256 |
...
| rendered tables | `dark-bench/tools/report.py` over a ledger snapshot | the report names the renderer's commit and the snapshot | commit and file |
